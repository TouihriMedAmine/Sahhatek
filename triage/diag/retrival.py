...
# Full KB - use paths relative to script directory
index_path = script_dir / "full_medical_index.faiss"
metadata_path = script_dir / "full_medical_metadata.pkl"

if not index_path.exists() or not metadata_path.exists():
    raise FileNotFoundError(
        f"FAISS indices not found in {script_dir}. "
        f"Please run diag/embedding.py first to build the indices."
    )
...
# Also try to load fast index from disk if it exists
fast_index_path = script_dir / "fast_medical_index.faiss"
fast_metadata_path = script_dir / "fast_medical_metadata.pkl"
if fast_index_path.exists() and fast_metadata_path.exists():
    try:
        fast_index = faiss.read_index(str(fast_index_path))
        with open(fast_metadata_path, "rb") as f:
            fast_metadata = pickle.load(f)
    except:
        pass  # Use the one we just created
