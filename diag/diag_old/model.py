...
def retrieve_conditions_expanded(query, negative_diseases, user_age_group=None, top_k=10):
    ...
    rag_docs = retrieve_conditions_faiss(query, user_age_group=user_age_group)  # Your existing FAISS retrieval
    filtered_docs = []
    for doc in rag_docs[:top_k]:  # Take more documents
        if doc.get("disease", "").lower() not in [d.lower() for d in negative_diseases]:
...
def generate_diagnosis(positive_symptoms, negative_symptoms, negative_diseases, user_age_group=None, expand_search=False):
    ...
    rag_context = retrieve_conditions_expanded(" ".join(positive_symptoms), negative_diseases, user_age_group, top_k)

    age_context = ""
    if user_age_group:
        age_context = f"Patient age group: {user_age_group}. Consider conditions more common in this age group but do not completely rule out other conditions."
...
Diseases ruled out: {list(negative_diseases)}
{age_context}
RAG medical knowledge: {rag_context}

Task:
- Identify top likely diagnoses. {"Consider less common conditions as well." if expand_search else ""}
- Use positive and negative symptoms to adjust confidence.
...
{{
  ...
  "diagnoses": [
    {{
      ...
      "explanation": "",
      "canonical_symptoms": [],
      "age_relevance": "high/medium/low"  # how relevant this condition is for patient's age group
    }}
  ]
}}
"""
    ...
    try:
        ...
        print("Error decoding diagnosis JSON:", response.choices[0].message.content)
        return None
